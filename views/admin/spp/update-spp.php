...
switch ($user["role"]) {
    ...
    if(isset($_POST["update"])) {
        $created = $TableSpp->updateSpp($_POST,$_GET["id"]);
        if($created > 0) {
            $success = true;
        } else {
            $error = true;
    ...
    <div class="max-w-md mx-auto bg-white rounded-lg overflow-hidden md:max-w-xl shadow-lg">
        <div class="md:flex">
            ...
            <form class="mt-6" action="" method="post">
                ...
                <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="full-name" name="nominal" value="<?= $spp["nominal"] ?>" type="text" placeholder="Nominal (RP)">
                </div>
