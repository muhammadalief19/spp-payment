...
if(isset($_POST["create"])) {
    $created = $TableKelas->createKelas($_POST);
    if($created > 0) {
        $success = true;
    } else {
        $error = true;
            ...
            <div class="mb-4 flex flex-col gap-2">
                ...
                <div class="mb-4 flex flex-col gap-2">
                    ...
                    <select class="appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="id_kompetensi" name="id_kompetensi">
                        ...
                        <option value="<?= $item["id_kompetensi"] ?>"><?= $item["nama"] ?></option>
                        <?php endforeach ?>
                    </select>
                    <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
                        <svg class="fill-current h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M10 12l-6-6h12z"/></svg>
                ...
                <div class="w-full flex items-center justify-end">
                    <button class="bg-blue-800 hover:bg-blue-700 text-white font-bold p-2 rounded-full focus:outline-none focus:shadow-outline" type="submit" name="create">
